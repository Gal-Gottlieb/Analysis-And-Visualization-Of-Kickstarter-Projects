...
-- 1) Verifying whether any column's NULL values exist in the database
...
-- 2) Looking for outliers in the columns Goal, Pledged, and Backers

select *
from kickstarter_project
where goal<=0 or Pledged<0 or Backers<0;

-- 3) Deleting outliers where Goal is equal to zero

delete from kickstarter_project
where goal=0;

-- 4) Checking for duplication of categories and subcategories

select distinct Category,Subcategory
from kickstarter_project
order by Category, Subcategory;

-- 5) Checking that there are no duplicate countries

select distinct(Country)
from kickstarter_project
order by Country;

-- 6) Making sure the primary key values (ID) do not repeat
...
-- 7) Adding constraints to the table's columns
