...
select 
	kp.name AS 'Name', count(*) AS 'Count'
from 
...
from kickstarter_project
where 
	name is null or Category is null or Projectstate is null or country is null or Subcategory is null or Launched is null
...
select 
	min(Launched) as 'Min Launched Date' ,max(Launched) as 'Max Launched Date'
from 
	kickstarter_project;

select 
	min(Deadline) as 'Min Deadline Date' ,max(Deadline) as 'Max Deadline Date'
from 
	kickstarter_project;
...
select 
	year(Launched) [Year],count(*) 'Number of Projects'
from 
	kickstarter_project
group by 
	year(Launched);
...
select 
	Country, datepart(hour, Launched) [time], count(*)
from 
	kickstarter_project
group by 
	country, datepart(hour, Launched)
order by 
	country, count(*) desc;
...
select 
	category,Subcategory,year(launched) [year] ,month(Launched) [Month],country,Goal,pledged,
...
where 
	goal>1000 
order by 
	[Goal completion] desc, Pledged desc;
...
select 
	name,category, subcategory, year(launched) [year],country,goal,pledged, round(Pledged/goal,0) as 'Goal completion' 
from 
	kickstarter_project
where 
	goal>1000
...
select 
	round(PERCENTILE_CONT(0.5) within group (order by pledged) over (),0) as 'Median Pledged', round(avg(pledged) over(),0)
...
select 
	distinct year(Launched) [Year] ,Category ,count(*) over (partition by year(Launched), category) as 'Number of projects'
...
select 
	distinct month(Launched) [Month] ,Category ,count(*) over (partition by month(Launched), category) as 'Number of projects'
...
(
	select 
		distinct Country,Category ,count(*) over (partition by Country, category) as 'Number of projects'
...
(
	select 
		distinct Category,subcategory ,count(*) over (partition by category,subcategory) as 'Number of projects'
...
select 
	q1.Country,Q1.Category,Q1.Subcategory,Q1.[AVG Succsesful campaign time in days],
	Q2.[AVG not successful campaign time in days]
from
(
	select  
		distinct country,Category,Subcategory,
...
select 
	count(*) as 'Count','USA' as 'USA or not'
from 
	kickstarter_project
where 
	country  like '%states'
...
select 
	distinct Country, datepart(hour, Launched) [time], Projectstate, count(*) over
	(partition by country, datepart(hour, Launched),projectstate) as 'Number of Projects'
from 
	kickstarter_project
where 
	country like 'united states' and Projectstate in ('successful','failed')
order by 
	time,Projectstate;
...
select 
	q1.Category,q1.Subcategory,q1.Country,q1.Year,q1.[Number of successful Projects],q2.[Number of failed Projects],
...
from(
	...
	from 
		kickstarter_project
	where  
		Projectstate like 'successful' and goal>1000 and year(Launched) not in ('2009','2018')
)
AS Q1
inner join
(
...
select 
	distinct year(Launched) as 'Year', Category, Projectstate ,avg(datediff(dd,Launched,Deadline)) over 
...
select 
	distinct Category,Subcategory, Projectstate ,avg(datediff(dd,Launched,Deadline)) over 
...
where 
	Projectstate in ('successful','failed')
order by 
	Category,Subcategory ,Projectstate;
...
select 
	distinct country, Category, Projectstate ,avg(datediff(dd,Launched,Deadline)) over 
...
select 
	year(Launched)[Year],Name,Category,Subcategory,Goal,round(PERCENTILE_CONT(0.5) within group (order by goal) 
...
order by 
	year(Launched), Category, Subcategory, [Goal level];
...
select 
	q1.Category,q1.Subcategory,q1.Country,q1.Year,q1.[Successful Projects],
...
(
	select 
		Category, Subcategory,country,year(Launched) [Year],count(*) as 'Successful Projects', 
	...
	from 
		kickstarter_project
	where 
		Projectstate='successful' and year(Launched) not in ('2009','2018')
	group by 
		Category, Subcategory,country,year(Launched)
) 
AS q1
inner join
(
select 
	Category, Subcategory,country,year(Launched) [Year],count(*) as 'Unsuccessful Projects'
from 
	kickstarter_project
where 
	Projectstate not like 'successful' and year(Launched) not in ('2009','2018')
group by 
	Category, Subcategory,country,year(Launched)
) 
AS q2
...
order by 
	Category,Subcategory,Country,Year;
...
from 
	kickstarter_project
where 
	Projectstate='successful' and year(Launched) not in ('2009','2018') and goal>1000;
